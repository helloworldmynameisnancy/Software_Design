...
@task
def test():
    sh('python3 -m coverage run --source src --omit src/game_of_life_gui.py -m unittest discover -s test')
...
@task
def run():
    sh('arch -arm64 brew install python-tk')
    sh('python3 -m src.game_of_life_gui')
