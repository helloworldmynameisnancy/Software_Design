...
@task
def setup():
    sh('python3 -m pip install -U coverage parameterized radon requests geopy us')

@task
def test():
    sh('python3 -m coverage run --source src --omit src/output_iss_details.py -m unittest discover -s test')
...
@needs('setup')
def run():
    sh('python3 -m src.output_iss_details')
